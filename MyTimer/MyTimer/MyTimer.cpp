...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	HDC hdc;
	PAINTSTRUCT ps;
	SYSTEMTIME st;		// 시간값을 조사할 구조체
	static TCHAR sTime[128];		// 시간값을 문자열로 변경하여 저장할 문자열, 조사해 놓은 시간을 기억해야 하므로 정적 변수로 선언
	...
	case WM_TIMER:		// wParam으로 타이머 ID를 전달받으며 lParam으로 타이머 메시지 발생시 호출될 함수의 번지가 전달된다.
		GetLocalTime(&st);		// 시간을 조사하는 함수
		wsprintf(sTime, TEXT("지금 시간은 %d:%d:%d입니다."), st.wHour, st.wMinute, st.wSecond);		// 출력을 위해 sTime 문자열로 변환해 둔다.
